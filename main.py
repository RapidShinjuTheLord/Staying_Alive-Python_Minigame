def main():
    ...
    class StayingAlive(Env):
        def __init__(self, will_render = False):
            ...
            #obs space
            self.observation_shape = (601,601,6)
        ...
        def draw_elements_on_canvas(self):
            ...
            self.canvas = np.zeros(self.observation_shape)*1
            self.canvas[int(self.player.position[0])][int(self.player.position[1])][0] = 1
            for danger in self.board.obstacles[::-1]:
                if danger.index == 'flame':
                    if danger.wall%2:
                        self.canvas[int(danger.getxy()[0])][int(danger.getxy()[1])][1] = danger.time/danger.initial_time
                    elif not danger.wall%2:
                        self.canvas[int(danger.getxy()[0])][int(danger.getxy()[1])][2] = danger.time/danger.initial_time
                elif danger.index == 'bomb':
                    self.canvas[int(danger.pos[0])][int(danger.pos[1])][3] = danger.time/danger.initial_time
                elif danger.index == 'laser':
                    if danger.axis:
                        self.canvas[0][int(danger.pos)][4] = danger.time/danger.initial_time
                    elif not danger.axis:
                        self.canvas[int(danger.pos)][0][4] = danger.time/danger.initial_time
                elif danger.index == 'plasma':
                    self.canvas[int(danger.pos[0])][int(danger.pos[1])][5] = 1
